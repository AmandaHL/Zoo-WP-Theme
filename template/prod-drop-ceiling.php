...
//drop ceiling product details
	...
	if( !empty( $prod_ducting ) ) {   
		echo '<div class="product-info">
			<div>
				<h4 class="product-subhead">Ducting</h4>'
				. wpautop($prod_ducting)
			.'</div>
		</div><!--.product-info-->';
	}
	if( !empty( $prod_diffuser ) ) {  
		echo '<div class="product-info">
			<div>
				<h4 class="product-subhead">Diffuser</h4>'
				. wpautop($prod_diffuser)
			.'</div>
		</div><!--.product-info-->';
	} 
	if( !empty( $prod_sound_char ) ) {  
		echo '<div class="product-info">
			<div>
				<h4 class="product-subhead">Sound Characteristics</h4>'
				. wpautop($prod_sound_char)
			.'</div>
		</div><!--.product-info-->';
	} 
	if( !empty( $prod_motor ) ) {   
